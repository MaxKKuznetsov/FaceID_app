# https://vc.ru/dev/70278-sozdat-sistemu-raspoznavaniya-lic-u-vhodnoy-dveri-za-150-s-pomoshchyu-odnoplatnogo-kompyutera-i-koda-na-python
...
from Utility.timer import elapsed_1arg
...
def lookup_known_face(face_encoding):
    ...
    if face_distances[best_match_index] < 0.6:
        ...
        # But we can say that if we have seen this person within the last 0.5 minutes, it is still the same
    ...
    while True:
        ...
        for metadata in known_face_metadata:
            ...
            if datetime.now() - metadata["last_seen"] < timedelta(seconds=10) and metadata["seen_frames"] > 5:
                ...
                face_distance = round(metadata["face_distance"], 2)
                visit_label1 = f"visits: {visits}"
                visit_label2 = f"face_distance: {face_distance}"

                if visits == 1:
                    visit_label = "First visit"
                cv2.putText(frame, visit_label1, (x_position + 10, 170), cv2.FONT_HERSHEY_DUPLEX, 0.6, (255, 255, 255),
                            1)
                cv2.putText(frame, visit_label2, (x_position + 10, 190), cv2.FONT_HERSHEY_DUPLEX, 0.6, (255, 255, 255),
                            1)
